async function fetchParaphrase(content: string, fullContent : string) {
      ...
      messages: [
        {
          role: 'system',
          content:
            `You are a helpful paraphraser. You will take the following text and rewrite it in a way that is easier to read and understand. The text is written in a formal style, and you should rewrite it in a more casual style. You should not change the meaning of the text, but you can add or remove information as needed to make it easier to read. You should also try to make the text shorter if possible. You should write the paraphrased text as if you were explaining it to a friend. You must only reply with the paraphrased text without any extra commenting or explanation. You should return the matching language of the original text. For example: Full text: Cats are very cute animals. Paraphrased text: cute animals. Result: adorable animals. Now, here is the text you need to paraphrase: ${content}. Here is the full paragraph: ${fullContent}`,
        },
        {
          role: 'user',
          content: 'Hello world',
